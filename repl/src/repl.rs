...
use crate::highlighter::CpHighlighter;
...
impl CpRepl {
    pub fn new() -> Self {
        ...
        let line_editor = Reedline::create()
            .with_highlighter(Box::new(CpHighlighter::new()))
            .with_validator(Box::new(CpValidator))
            .with_completer(completer)
            .with_menu(ReedlineMenu::EngineCompleter(completion_menu))
            .with_edit_mode(edit_mode);

        CpRepl { line_editor, counter: 0 }
    }

    pub fn iterate(&mut self) -> String {
        loop {
            let prompt = DefaultPrompt {
                left_prompt: DefaultPromptSegment::Basic(" cp ".into()),
                right_prompt: DefaultPromptSegment::Basic(format!(" -<{}>- ", self.counter)),
            };
            match self.line_editor.read_line(&CpPrompt(prompt)) {
...
pub struct CpPrompt(DefaultPrompt);

impl Prompt for CpPrompt {
...
pub struct CpValidator;

impl Validator for CpValidator {
...
#[cfg(test)]
mod tests {
    use crate::repl::CpValidator;
    use reedline::{ValidationResult, Validator};

    fn is_line_complete(line: &str) -> bool {
        let validator = CpValidator;
